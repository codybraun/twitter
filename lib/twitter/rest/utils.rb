  ...
  module REST
    ...
    private
      ...
      def merge_user!(hash, user, prefix = nil)
        ...
        when String
          set_compound_key("username", user, hash, prefix)
        when URI, Addressable::URI
          set_compound_key("username", user.path.split("/").last, hash, prefix)
        when Twitter::User
      ...
      def merge_users!(hash, users)
        ...
        hash[:username] = usernames.join(",") unless usernames.empty?
      end
