...
module Twitter
  module REST
    module Tweets
      ...
      def update!(status, options = {})
        hash = options.dup
        unless hash[:in_reply_to_status].nil?
          hash[:reply] = {:in_reply_to_tweet_id => hash.delete(:in_reply_to_status).id}
        end
        unless hash[:in_reply_to_status_id].nil?
          hash[:reply] = {:in_reply_to_tweet_id => hash.delete(:in_reply_to_status_id)}
        end
        hash[:place_id] = hash.delete(:place).woeid unless hash[:place].nil?
        perform_post_with_object("/2/tweets", hash.merge(text: status), Twitter::Tweet)
      end
      ...
      def update_with_media(status, media, options = {})
        ...
        media_ids = pmap(array_wrap(media)) do |medium|
          upload(medium)[:media_id].to_s
        end
        update!(status, options.merge({media: {media_ids: media_ids}}))
      end
