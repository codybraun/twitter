...
describe Twitter::Error do
  ...
  %w[error errors].each do |key|
    context "when JSON body contains #{key}" do
      before do
        ...
        stub_get("/2/statuses/user_timeline.json").with(query: {username: "sferik"}).to_return(status: 500, body: body, headers: {content_type: "application/json; charset=utf-8"})
      end
  ...
  Twitter::Error::ERRORS.each do |status, exception|
    context "when HTTP status is #{status}" do
      before do
        stub_get("/2/statuses/user_timeline.json").with(query: {username: "sferik"}).to_return(status: status, body: "{}", headers: {content_type: "application/json; charset=utf-8"})
      end
